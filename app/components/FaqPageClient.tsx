...
const FaqPageClient = ({ slug }: { slug: string }) => {
	...
	const [selectedOrganFromSlider, setSelectedOrganFromSlider] = useState<string>('');
	const router = useRouter();
	useEffect(() => {
		const fetchCourseMetaData = async () => {
			try {
				const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/course_meta_data/slug/${slug}`);
				...
				<>
					...
					<main className='flex flex-col items-center container mx-auto px-4 py-8'>
						{selectedOrganFromSlider !== '' && (
							<h1 className='md:pt-[60px] text-white text-center mt-10 text-[64px] md:text-8xl font-pnu font-bold !leading-snug'>
								{selectedOrganFromSlider}
							</h1>
						)}
